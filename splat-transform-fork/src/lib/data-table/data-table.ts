...
class DataTable {
    ...
    constructor(columns: Column[], transform?: Transform, pipelineMetadata?: PipelineMetadata) {
        if (columns.length === 0) {
        ...
        this.pipelineMetadata = pipelineMetadata ? { ...pipelineMetadata } : {};
    }
    ...
    clone(options?: { rows?: Uint32Array | number[]; columns?: string[] }): DataTable {
        ...
        if (!rows) {
            return new DataTable(srcColumns.map(c => c.clone()), this.transform, this.pipelineMetadata);
        }
        ...
        }), this.transform, this.pipelineMetadata);
...
export { Column, DataTable, TypedArray, ColumnType, Row, PipelineMetadata };
