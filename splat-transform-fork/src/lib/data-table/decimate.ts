...
const computeFrustumPlanes = (
    ...
    const right: [number, number, number] = [r00, r10, r20];
    const up: [number, number, number] = [r01, r11, r21];
    const forward: [number, number, number] = [r02, r12, r22];
    ...
    const toWorldNormal = (lx: number, ly: number, lz: number): [number, number, number] => {
        // basis @ local (basis columns are right, up, forward)
        return [
    ...
    const leftNormal = toWorldNormal(1, 0, tanX);
    const rightNormal = toWorldNormal(-1, 0, tanX);
    const bottomNormal = toWorldNormal(0, 1, tanY);
    const topNormal = toWorldNormal(0, -1, tanY);

    const leftPlane = normalizePlane(leftNormal[0], leftNormal[1], leftNormal[2], px, py, pz);
    const rightPlane = normalizePlane(rightNormal[0], rightNormal[1], rightNormal[2], px, py, pz);
    const bottomPlane = normalizePlane(bottomNormal[0], bottomNormal[1], bottomNormal[2], px, py, pz);
    const topPlane = normalizePlane(topNormal[0], topNormal[1], topNormal[2], px, py, pz);
...
const computePathAwareImportanceV5 = (
    ...
    if (poses.length === 0) {
        throw new Error('filterByPath requires at least one pose');
    }
